...
> [!IMPORTANT]
> 本功能不会下载或归档画廊内容、压缩包或种子，也不调用 archiver、消耗 GP。选择 EH/EX 搜索后，结果列表会由浏览器直接向经过校验的 EH 官方 HTTPS 图片域名加载结果中提供的封面缩略图；即使之后选择跳过封面，这一步仍会暴露客户端 IP。应用用户选中的元数据且选择更新封面时，服务端会另外下载所选画廊的一张封面并写入本地缩略图缓存。当前版本不提供反向图片搜索。
...
| 包含 `[gid]` 的标题（6-12 位数字） | 先用 `gid:{id}` 搜索找到 gallery token，再读取元数据；该 gid 搜不到时按序落到去掉方括号后的整条标题搜索。`[2021]`、`[01]` 这类年份和卷号不当作 gid，直接按标题搜索（LANraragi 的 `\[([0-9]+)\]` 不限位数，但它把 gid 搜索做成可选开关；本实现默认启用，因此用位数下限代替开关） |
| 完整 gallery URL | 直接读取该 `gid/token`，跳过搜索页面 |
| 漫画已有 `source:` gallery 标签 | 沿用作品当前标题搜索时，自动使用标签中的 `gid/token` 跳过标题搜索；该画廊已被删除或 token 失效时回落到标题搜索。改写搜索词即按新标题搜索，用于纠正上一次选错的画廊 |
