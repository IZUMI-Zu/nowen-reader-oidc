...
> [!IMPORTANT]
> This feature does not download gallery content, gallery archives, or torrents, and it does not call the archiver or spend GP. After an EH/EX search, the browser loads available result cover thumbnails directly from validated official EH HTTPS image hosts; this exposes the client IP even if the cover is later skipped. When selected metadata is applied with the cover enabled, the server separately downloads one cover from the selected gallery into the local thumbnail cache. Reverse-image search is not included in this version.
