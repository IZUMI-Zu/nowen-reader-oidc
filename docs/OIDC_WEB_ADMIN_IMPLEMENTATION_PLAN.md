...
- 中英文 UI、错误码、配置及恢复文档。
- 既有用户管理接口的防锁死约束：删除或降级最后一个绑定当前 issuer 的管理员时返回 `409 oidc_admin_required`（见 11.4）。
- 本地 break-glass 密码的长度约束按 bcrypt 上限校验，避免恢复密码在设置时静默截断。
...
关闭密码登录后，用户管理接口本身也会成为锁死入口，因此 `PUT /api/auth/users` 与
`DELETE /api/auth/users` 同样受约束：

- 删除或把角色降级会移除最后一个绑定当前 issuer 的管理员时，返回 `409 oidc_admin_required`。
- 该检查与实际写入在同一事务内重复执行，并发请求不能绕过。
- 存储密钥不可用导致 issuer 解析失败时，保护按已保存的 issuer 生效，不退化为放行。
