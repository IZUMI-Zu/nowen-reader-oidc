...
func TestPasswordCreationRoutesEnforceCharacterAndBcryptLimits(t *testing.T) {
	r := setupTestRouter(t)
	shortUnicodePassword := "密码"
	overlongASCII := strings.Repeat("a", 73)
	overlongUnicode := strings.Repeat("密", 25)
	...
	register = performRequest(r, http.MethodPost, "/api/auth/register", map[string]string{
		"username": "long-password-user", "password": overlongASCII,
	})
	if register.Code != http.StatusBadRequest {
		t.Fatalf("register 73-byte password status = %d: %s", register.Code, register.Body.String())
	}
	...
	createUser = performAuthedRequest(r, http.MethodPost, "/api/auth/users", map[string]string{
		"username": "unicode-too-long", "password": overlongUnicode, "role": "user",
	}, adminCookie)
	if createUser.Code != http.StatusBadRequest {
		t.Fatalf("admin create 75-byte Unicode password status = %d: %s", createUser.Code, createUser.Body.String())
	}
	...
	exactBcryptLimit := performAuthedRequest(r, http.MethodPost, "/api/auth/users", map[string]string{
		"username": "exact-byte-limit", "password": strings.Repeat("b", 72), "role": "user",
	}, adminCookie)
	if exactBcryptLimit.Code != http.StatusOK {
		t.Fatalf("admin create 72-byte password status = %d: %s", exactBcryptLimit.Code, exactBcryptLimit.Body.String())
	}
	...
	changePassword = performAuthedRequest(r, http.MethodPut, "/api/auth/users", map[string]string{
		"action": "changePassword", "oldPassword": "password123", "newPassword": overlongASCII,
	}, adminCookie)
	if changePassword.Code != http.StatusBadRequest {
		t.Fatalf("change 73-byte password status = %d: %s", changePassword.Code, changePassword.Body.String())
	}
	...
	setInitial = performAuthedRequest(r, http.MethodPost, "/api/auth/password", map[string]string{
		"newPassword": overlongUnicode,
	}, "unicode-oidc-session")
	if setInitial.Code != http.StatusBadRequest {
		t.Fatalf("initial 75-byte Unicode password status = %d: %s", setInitial.Code, setInitial.Body.String())
	}
